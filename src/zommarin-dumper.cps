...
function dumpComment(name, value) {
  writeln("//  " + name + " = " + value);
}
...
function onMachine() {
  ...
  if (machineConfiguration.getVendor()) {
    dumpComment("Vendor", machineConfiguration.getVendor());
  }
  if (machineConfiguration.getModel()) {
    dumpComment("Model", machineConfiguration.getModel());
  }
  if (machineConfiguration.getDescription()) {
    dumpComment("Description", machineConfiguration.getDescription());
  }
}

function onOpen() {
  dump("onOpen", arguments);
  dumpComment("Post Engine Version", getVersion());
  dumpComment("Program Name", programName);
  dumpComment("Program Comment", programComment);
}
